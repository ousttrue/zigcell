...
fn nodeSourceRange(tree: Ast, node: Ast.Node.Index) SourceRange {
    ...
    const loc_end = tokenLocation(tree, ast.lastToken(tree, node));
    ...
    switch (node_tag) {
        ...
        => {
            ...
            if (token_tags[first_token] == .identifier) {
                ...
                scope.* = .{
                    .range = .{
                        .start = tokenLocation(tree, main_tokens[node_idx]).start,
                        .end = tokenLocation(tree, last_token).start,
                    },
                    .decls = std.StringHashMap(Declaration).init(allocator),
                    .data = .other,
                };
        ...
        => {
            ...
            if (if_node.payload_token) |payload| {
                ...
                scope.* = .{
                    .range = .{
                        .start = tokenLocation(tree, payload).start,
                        .end = tokenLocation(tree, ast.lastToken(tree, if_node.ast.then_expr)).end,
                    },
                    .decls = std.StringHashMap(Declaration).init(allocator),
                    .data = .other,
                };
            ...
            if (if_node.ast.else_expr != 0) {
                if (if_node.error_token) |err_token| {
                    ...
                    scope.* = .{
                        .range = .{
                            .start = tokenLocation(tree, err_token).start,
                            .end = tokenLocation(tree, ast.lastToken(tree, if_node.ast.else_expr)).end,
                        },
                        .decls = std.StringHashMap(Declaration).init(allocator),
                        .data = .other,
                    };
        ...
        .@"catch" => {
            ...
            scope.* = .{
                .range = .{
                    .start = tokenLocation(tree, tree.firstToken(catch_expr)).start,
                    .end = tokenLocation(tree, ast.lastToken(tree, catch_expr)).end,
                },
                .decls = std.StringHashMap(Declaration).init(allocator),
                .data = .other,
            };
        ...
        => {
            ...
            if (while_node.label_token) |label| {
                ...
                scope.* = .{
                    .range = .{
                        .start = tokenLocation(tree, while_node.ast.while_token).start,
                        .end = tokenLocation(tree, ast.lastToken(tree, node_idx)).end,
                    },
                    .decls = std.StringHashMap(Declaration).init(allocator),
                    .data = .other,
                };
            ...
            if (while_node.payload_token) |payload| {
                ...
                scope.* = .{
                    .range = .{
                        .start = tokenLocation(tree, payload).start,
                        .end = tokenLocation(tree, ast.lastToken(tree, while_node.ast.then_expr)).end,
                    },
                    .decls = std.StringHashMap(Declaration).init(allocator),
                    .data = .other,
                };
            ...
            if (while_node.ast.else_expr != 0) {
                if (while_node.error_token) |err_token| {
                    ...
                    scope.* = .{
                        .range = .{
                            .start = tokenLocation(tree, err_token).start,
                            .end = tokenLocation(tree, ast.lastToken(tree, while_node.ast.else_expr)).end,
                        },
                        .decls = std.StringHashMap(Declaration).init(allocator),
                        .data = .other,
                    };
            ...
            for (cases) |case| {
                ...
                if (switch_case.payload_token) |payload| {
                    ...
                    scope.* = .{
                        .range = .{
                            .start = tokenLocation(tree, payload).start,
                            .end = tokenLocation(tree, ast.lastToken(tree, switch_case.ast.target_expr)).end,
                        },
                        .decls = std.StringHashMap(Declaration).init(allocator),
                        .data = .other,
                    };
        ...
        .@"errdefer" => {
            ...
            if (data[node_idx].lhs != 0) {
                ...
                scope.* = .{
                    .range = .{
                        .start = tokenLocation(tree, payload_token).start,
                        .end = tokenLocation(tree, ast.lastToken(tree, expr)).end,
                    },
                    .decls = std.StringHashMap(Declaration).init(allocator),
                    .data = .other,
                };
